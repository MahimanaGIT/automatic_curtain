...
Storage::Storage(Logging *logging) : logger_(logging) {}
...
bool Storage::saveDeviceCred(const CONFIG_SET::DEVICE_CRED *device_cred) {
  ...
  size_t status_ssid =
      preferences_.putString(CONFIG_SET::KEY_SSID.c_str(), device_cred->SSID);
  size_t status_pass = preferences_.putString(CONFIG_SET::KEY_PASSWORD.c_str(),
                                              device_cred->PASSWORD);
  preferences_.end();
...
bool Storage::populateDeviceCred(CONFIG_SET::DEVICE_CRED *device_cred) {
  preferences_.begin(CONFIG_SET::STORAGE_NAMESPACE.c_str(), false);
  device_cred->DEVICE_ID =
      preferences_.getString(CONFIG_SET::KEY_DEVICE_ID.c_str(), "");
  device_cred->SSID = preferences_.getString(CONFIG_SET::KEY_SSID.c_str(), "");
  device_cred->PASSWORD =
      preferences_.getString(CONFIG_SET::KEY_PASSWORD.c_str(), "");
  preferences_.end();
...
bool Storage::saveCalibParam(const CONFIG_SET::CALIB_PARAMS *calib_param) {
  preferences_.begin(CONFIG_SET::STORAGE_NAMESPACE.c_str(), false);
  size_t status_stall = preferences_.putInt(CONFIG_SET::KEY_STALL_VALUE.c_str(),
                                            calib_param->STALL_VALUE);
  size_t status_total_step = preferences_.putInt(
      CONFIG_SET::KEY_TOTAL_STEP_COUNT.c_str(), calib_param->TOTAL_STEP_COUNT);
  preferences_.end();
...
bool Storage::populateCalibParam(CONFIG_SET::CALIB_PARAMS *calib_param) {
  preferences_.begin(CONFIG_SET::STORAGE_NAMESPACE.c_str(), false);
  calib_param->TOTAL_STEP_COUNT =
      preferences_.getInt(CONFIG_SET::KEY_TOTAL_STEP_COUNT.c_str(), -1);
  calib_param->STALL_VALUE =
      preferences_.getInt(CONFIG_SET::KEY_STALL_VALUE.c_str(), -1);
  preferences_.end();
